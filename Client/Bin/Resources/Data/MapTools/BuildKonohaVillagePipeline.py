...
def build_ground_collision_stage(mesh_src: Path, mesh_dst: Path) -> tuple[Path | None, Path | None]:
    ground_src = mesh_src / "Ground_Collision"
    if not ground_src.exists():
        return None, None

    stage_root = mesh_dst.parent / "__GroundCollisionStage"
    if stage_root.exists():
        shutil.rmtree(stage_root, ignore_errors=True)
    stage_root.mkdir(parents=True, exist_ok=True)

    staged_any = False
    for gltf_path in sorted(ground_src.glob("COL_*.gltf")):
        staged_gltf_name = gltf_path.name.removeprefix("COL_")
        shutil.copy2(gltf_path, stage_root / staged_gltf_name)

        source_bin_path = gltf_path.with_suffix(".bin")
        if source_bin_path.exists():
            shutil.copy2(source_bin_path, stage_root / source_bin_path.name)
    ...
    ground_dst = mesh_dst / "Ground_Collision"
    if ground_dst.exists():
        shutil.rmtree(ground_dst, ignore_errors=True)
    ground_dst.mkdir(parents=True, exist_ok=True)
    return stage_root, ground_dst


# Normalize generated collision filenames so the runtime folder matches KonohaVillage02 style.
def normalize_ground_collision_outputs(ground_dst: Path):
    for path in sorted(ground_dst.iterdir()):
        if not path.is_file():
            continue

        if not path.name.startswith("COL_"):
            continue

        renamed_path = path.with_name(path.name.removeprefix("COL_"))
...
# Convert staged ground-only collision meshes into the runtime Ground_Collision folder.
def convert_ground_collision_meshes(args):
    stage_root, ground_dst = build_ground_collision_stage(args.mesh_src, args.mesh_dst)
    if stage_root is None or ground_dst is None:
        print("[GROUND COLLISION] skipped: no COL_*.gltf files found")
        return
    ...
    if args.dry_run:
        print("[GROUND COLLISION] dry-run skipped")
        print(" ".join(f"\"{item}\"" if " " in item else item for item in ground_command))
        return

    run_command(ground_command, "GROUND COLLISION")
    normalize_ground_collision_outputs(ground_dst)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Build the KonohaVillage mesh/material/level pipeline in one shot."
    )
...
def main():
    ...
    print("")
    print("[BuildKonohaVillagePipeline] Finished.")
