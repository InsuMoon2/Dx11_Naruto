...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance,
    ...
    wstring args =
        (firstSpace != wstring::npos) ? cmdStr.substr(firstSpace + 1) : L"";
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow,
    ...
    wstring windowTitle =
        wstring(szTitle) + L" - Player " + to_wstring(params.playerIndex + 1);

    RECT rc = { 0, 0, static_cast<LONG>(params.windowWidth),
               static_cast<LONG>(params.windowHeight) };
    ...
    HWND hWnd = CreateWindowW(szWindowClass, windowTitle.c_str(),
        WS_OVERLAPPEDWINDOW, posX, posY, windowWidth,
