...
static string Resolve_AnimNotifyPropertyJsonKey(const Engine::FPropertyInfo& prop)
{
...
json AnimNotify::Serialize_Payload() const
{
    ...
    auto& info = const_cast<AnimNotify*>(this)->Get_ReflectionInfo();
    const char* basePtr = reinterpret_cast<const char*>(this);

    for (const auto& prop : info.properties)
    {
        const string key = Resolve_AnimNotifyPropertyJsonKey(prop);
