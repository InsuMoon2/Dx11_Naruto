...
VS_OUT VS_MAIN(VS_IN In)
{
    ...
    float4 localPos = mul(float4(In.vPosition, 1.f), matBone);
    float3 localNormal = normalize(mul(float4(In.vNormal, 0.f), matBone).xyz);
    float3 localTangent = normalize(mul(float4(In.vTangent, 0.f), matBone).xyz);

    float4 worldPos = mul(localPos, g_WorldMatrix);
    float4 viewPos = mul(worldPos, g_ViewMatrix);

    Out.vPosition = mul(viewPos, g_ProjMatrix);
    Out.vNormal = normalize(mul(float4(localNormal, 0.f), g_WorldMatrix).xyz);
    Out.vTangent = normalize(mul(float4(localTangent, 0.f), g_WorldMatrix).xyz);
