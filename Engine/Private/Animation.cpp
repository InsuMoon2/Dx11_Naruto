...
void Animation::Rebind_BoneIndices(const vector<Shared<Bone>>& bones)
{
    if (_channels.empty())
        return;

    unordered_map<string, int32> boneNameToIndex;
    ...
    for (const auto& channel : _channels)
    {
        ...
        const auto it = boneNameToIndex.find(channel->Get_NodeName());
        ...
        channel->Set_BoneIndex(it->second);
    }
}
